...
def find_block_lines(lines, key_to_remove):
    ...
    if start_line_idx == -1:
        ...
        key_pattern_no_brace = re.compile(
            r'^\s*"' + re.escape(key_to_remove) + r'"\s*:\s*$'
        )
        for i, line in enumerate(lines):
...
def remove_block_surgically(file_path, key_to_remove):
    ...
    try:
        ...
        print(
            f"Error writing updated data to {file_path} after removing {key_to_remove}: {e}"
        )
        return False
...
def main():
    ...
    if common_keys:
        ...
        for key in common_keys:
            ...
            aider_json_filtered = [
                line for line in aider_json_lines if dummy_key not in line
            ]
            ...
            diff = difflib.unified_diff(
                ...
                lineterm="",
                n=max(
                    len(litellm_json_filtered), len(aider_json_filtered)
                ),  # Show all lines
            )
