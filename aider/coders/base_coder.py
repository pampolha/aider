...
class Coder:
    ...
    def get_announcements(self):
        lines = []
        lines.append(f"pampoAider🥸 v{__version__}")
        ...
        if rag_fnames:
            ...
            for fname in rag_fnames:
                ...
                if os.path.exists(abs_fname):
                    ...
                    self.io.tool_warning(
                        f"Error: RAG file {fname} does not exist. Skipping."
                    )
    ...
    def get_rag_file_messages(self, rag_query_results: list[Document] | None):
        ...
        rag_messages += [
            dict(role="user", content=self.gpt_prompts.rag_files_prefix + rag_content),
            dict(
                role="assistant",
